...
If you're thinking about putting Blazor into an Optimizely CMS 13 project, this is the post I'd have wanted first.
...
Building that over MVC and polling endpoints means hand-writing the diffing, the "only fetch lines newer than X" bookkeeping, and the DOM updates in JavaScript — against a CMS shell that already has opinions about the page. Blazor Server gives you the diffing for free, over a connection that's already open.

There was a second reason, and I'll be honest about it. CMS 13 moved Optimizely onto .NET 10, and Blazor is now simply part of the platform the CMS runs on.
...
Components that live inside a NuGet package are invisible to that check. The SDK sees no `.razor` files, doesn't reference the pack, and the file is never served. Your page renders perfectly, looks finished, and does absolutely nothing. The only clue is a 404 for `blazor.server.js` in the browser console.
...
Applications that already have their own `.razor` files never hit this, which is exactly why it's easy to miss during development and easy to hit on a clean host.
...
This is the one that came in from a real site — CMS 13 with Commerce 15 — and it's the reason the setup instructions look the way they do.
...
If your application already owns `/_blazor`, the package detects the existing mapping and skips its own. The one case detection can't see is a host that maps its hub **after** this call, so there's an explicit escape hatch:

```csharp
options.MapBlazorHub = false;   // your application owns /_blazor
```

One detail worth keeping: endpoint matching runs *before* authentication. So this reproduces on an anonymous request, and you don't need a CMS login to diagnose it. That turns a "works on my machine, breaks in the client's environment" problem into a curl command.

## Prerendering and time zones

Timestamps in an admin UI should be in the reader's own time zone. In a prerendered Blazor app, that's harder than it sounds.

The usual approach is to ask the browser for its zone. But prerendering happens on the server, before any browser code runs, so the first paint is in the wrong zone and then flickers into place when the circuit connects. On a page that's mostly a table of timestamps, that flicker is the whole page moving.
...
The trade-off is real and I decided to state it rather than hide it. The very first page view renders in UTC, and says so — the cookie is written by that page and doesn't exist until it has been served once. Nothing reloads to paper over that single occurrence.
...
A running execution re-reads itself every two seconds and appends only lines newer than those already shown, so following a long job stays cheap. Leave the page open while the job finishes and it catches up on its own — the badge flips, the duration fills in, and sections that didn't exist yet appear.
...
The page reads once more a moment after the run ends, which picks up that final batch. Without it, a job would occasionally finish on screen with its last few log lines missing — the kind of bug that's very hard to reproduce deliberately and very obvious to the person watching.

## Wrapping up

Blazor inside the Optimizely CMS shell works, and for a live, stateful admin screen it's genuinely less work than the MVC equivalent. But the hosting details are where the time goes, not the components. Two of my three problems were about endpoints and build targets, not about Razor at all.
...
Have you put Blazor into an Optimizely project? I'd like to know what else bites. Let me know in the comments. Thank you for reading!
