...
All changes described below are available on [my GitHub repository](https://github.com/szolkowski/MyOptiAlloySite). You can also review the complete list of changes in [this pull request](https://github.com/szolkowski/MyOptiAlloySite/pull/1).
...
The original `db.dockerfile` uses `mcr.microsoft.com/mssql/server:2019-latest`. Neither SQL Server 2019 nor 2025 have native ARM images — both run under Rosetta 2 (x64 emulation) on Apple Silicon. However, SQL Server 2019 is particularly unreliable in this setup, while SQL Server 2025 (CU1+) works well under emulation after Microsoft fixed an AVX instruction issue that caused the RTM image to crash on ARM. Switching to `2025-latest` gives us a much more stable development experience.
...
1. **Delete stale LocalDB files** — remove `.mdf`/`.ldf` files from `App_Data/` generated by the template
2. **Upgrade SQL Server** — switch from `2019-latest` to `2025-latest` for stable Rosetta 2 emulation on ARM
